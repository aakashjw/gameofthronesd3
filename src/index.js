...
// Assignment 4
// This is a Force Driven Graph example using D3 and JavaScript.
...
//
// Visualization is done using froce driven graph to show the following:
//    1. Top Contenders for Iron Throne
//    2. Contender's council members
//
// There are various functionalities supported by the code:
// 1. Nodes are attached to an image of the character.
// 2.Hover Function
...
var div1 = d3.select("body").append("div").style("top","-110px").style("left","-10px").style("opacity", 1);
...
//Decrepition Box to show their house
...
// Setup SVG Area
//
var svg = d3.select("body").append("svg").attr("width", 1020).attr("height", 800),
  ...
  var setEvent = images
   ...
   //display msg about the selected node on hover
      ...
      div2.html(hoverObj.name + "<br/>" +  "Status: "+hoverObj.status).style("left" ,(hoverObj.x+100) +"px")
         .style("top", (hoverObj.y +230) + "px").style("background",hoverObj.h_color);
